...
pub fn base58_encode(data: &[u8]) -> String {
    base58_encode(data).into_string()
}

pub fn base58_decode(data: &str) -> Vec<u8> {
    base58_decode(data).into_vec().unwrap()
}

pub fn ecdsa_p256_sha256_sign_digest(pkcs8: &[u8], message: &[u8]) -> Vec<u8> {
    let key_piar = EcdsaKeyPair::from_pkcs8(&ECDSA_P256_SHA256_FIXED_SIGNING,pkcs8).unwrap();
    let rng = ring::rand::SystemRandom::new();
    key_piar.sign(&rng, message).unwrap().as_ref().to_vec()
}

pub fn ecdsa_p256_sdha256_sign_verify(public_key: &[u8], signature: &[u8], message: &[u8]) -> bool {
